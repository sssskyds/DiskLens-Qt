...
void LargeFilesView::setupUI() {
    ...
    m_limitCombo->setCurrentIndex(1);
...
void LargeFilesView::onDeleteClicked() {
    ...
    if (reply == QMessageBox::Yes) {
        OperationResult res = m_fileOpService.moveToTrash(file.path);
        if (res.success) {
...
void LargeFilesView::onCustomContextMenu(const QPoint& pos) {
    ...
    if (selected == openAct) {
        ...
        auto reply = QMessageBox::warning(this, "PERMANENT DELETE?",
                                           ...
                                           QMessageBox::Yes | QMessageBox::No);
        if (reply == QMessageBox::Yes) {
            OperationResult res = m_fileOpService.deletePermanently(file.path);
            if (res.success) {
