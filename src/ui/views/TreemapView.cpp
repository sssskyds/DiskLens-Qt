...
void TreemapView::setupUI() {
    ...
    // Bottom Selected Details Panel (Glassmorphism Card)
...
void TreemapView::onNodeClicked(std::shared_ptr<FileSystemNode> node) {
    ...
    // Don't allow trashing the root drive folder itself
...
void TreemapView::onDeleteClicked() {
    ...
    if (reply == QMessageBox::Yes) {
        bool ok = m_fileOpService.moveToTrash(m_selectedNode->getPath());
        if (ok) {
            QMessageBox::information(this, "Success", "Moved to Recycle Bin successfully.");
            // Reset and clear selection
            clear();
            // In a complete tree model, we would remove the node and re-render the layout
            // For now, simple reload/refresh is recommended, but let's just redraw
