...
void DuplicatesView::setupUI() {
    ...
    m_wastedSpaceLabel->setStyleSheet("font-size: 16px; font-weight: bold; color: #f85149;");
...
void DuplicatesView::populateTree() {
    ...
    for (const auto& group : m_duplicateGroups) {
        ...
        parentItem->setText(0, QString("Group MD5 Hash [%1]").arg(group.fullHash.left(12)));
...
void DuplicatesView::onDeleteSelected() {
    ...
    if (reply == QMessageBox::Yes) {
        ...
        for (const QString& path : filesToDelete) {
            OperationResult res = m_fileOpService.moveToTrash(path);
            if (res.success) {
                successCount++;
            }
        }
